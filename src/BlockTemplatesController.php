...
class BlockTemplatesController {
	...
	protected function migrate_page( $template_slug, $page ) {
		...
		$template_content    = BlockTemplateMigrationUtils::get_page_template( $page );
		$has_default_content = preg_replace( '/\s+/', '', $page->post_content ) === preg_replace( '/\s+/', '', '<!-- wp:shortcode -->[woocommerce_' . $template_slug . ']<!-- /wp:shortcode -->' );

		if ( ! $template_content && $has_default_content ) {
			// Nothing to migrate.
			return $this->set_has_migrated_page( $template_slug );
		}

		if ( ! $template_content ) {
			$template_content = BlockTemplateMigrationUtils::get_default_template( $page, $template_slug );
		}
