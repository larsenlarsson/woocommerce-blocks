...
class BlockTemplateMigrationUtils {
	...
	public static function get_page_template( $page ) {
		...
		$block_template_content = $block_template->content;

		if ( stristr( $block_template_content, 'wp:post-content' ) ) {
			...
			$replacement            = '
				...
				<!-- /wp:group -->
			';
			$block_template_content = preg_replace( $pattern, $replacement, $block_template_content );
		}
	 ...
	 * @return string
	 */
	public static function get_default_template( $page, $template_slug ) {
